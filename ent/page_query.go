...
// Returns a *NotSingularError when exactly one Page entity is not found.
...
func (pq *PageQuery) Clone() *PageQuery {
	...
	return &PageQuery{
		...
		withPageOf: pq.withPageOf.Clone(),
...
func (pq *PageQuery) sqlAll(ctx context.Context) ([]*Page, error) {
	...
	if query := pq.withPageOf; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_pages" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.PageOf = n
			}
		}
	}
