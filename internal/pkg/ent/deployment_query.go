...
type DeploymentQuery struct {
	...
	// eager-loading edges.
	withDeploymentsOf *ProjectQuery
	withFKs           bool
...
// QueryDeploymentsOf chains the current query on the "deployments_of" edge.
func (dq *DeploymentQuery) QueryDeploymentsOf() *ProjectQuery {
...
func (dq *DeploymentQuery) Clone() *DeploymentQuery {
	...
	return &DeploymentQuery{
		...
		withDeploymentsOf: dq.withDeploymentsOf.Clone(),
...
// WithDeploymentsOf tells the query-builder to eager-load the nodes that are connected to
// the "deployments_of" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DeploymentQuery) WithDeploymentsOf(opts ...func(*ProjectQuery)) *DeploymentQuery {
	query := &ProjectQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withDeploymentsOf = query
	return dq
}
...
func (dq *DeploymentQuery) sqlAll(ctx context.Context) ([]*Deployment, error) {
	var (
		...
		_spec       = dq.querySpec()
		loadedTypes = [1]bool{
			dq.withDeploymentsOf != nil,
		}
	)
	if dq.withDeploymentsOf != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withDeploymentsOf; query != nil {
